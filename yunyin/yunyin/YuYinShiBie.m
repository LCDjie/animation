...
@implementation YuYinShiBie

static YuYinShiBie *manager = nil;
...
- (void)playVoice:(NSString *)str{
    ...
    //设置语音合成的参数
    [iFlySpeechSynthesizer setParameter:@"speed" value:@"50"];  //合成的语速，取值范围0 -- 100
    [iFlySpeechSynthesizer setParameter:@"volume" value:@"50"]; //合成的音量，取值范围0 -- 100
    //发音人，为 “ xiaoqian ”可以设置其他的
    [iFlySpeechSynthesizer setParameter:@"voice_name" value:@"xiaoyan"];
    [iFlySpeechSynthesizer setParameter:@"smaple_rate" value:@"8000"];  //音频采样率，目前支持采样率有16000 和 8000
    
    [iFlySpeechSynthesizer startSpeaking:str];    //开始发音
...
- (void)discernBlock:(void (^)(NSString *))a{
    //block输入值
    self.onResult = a;
    iflySpeechRecognizer = [IFlySpeechRecognizer createRecognizer: @"appid=52bbb432,timeout=10000" delegate:self];//时间按照毫秒计算
...
- (void)onResults:(NSArray *)results{
    ...
    for(NSString *key in dic){
        ...
        //[result appendFormat:@"%@ (置信度：%@) ",key,value];
